...
The response should rewrite `dist.tarball` URLs to the concrete selected mount:
...
tarball URLs in packuments so origin identity remains explicit after the
packument is served.
...
- private or pnpr-hosted routes use pnpr mount URLs;
- fallback/group routes emit tarball URLs for the selected concrete mount.
...
tarball cache namespaces, and pnpr-routed tarball URLs. This avoids having one
concept for resolver caching and a different concept for registry serving.
...
5. Make fallback/group mounts select a concrete child mount for packuments and
   rewrite tarball URLs to that child mount by default.
6. Teach the resolver allowlist and route classifier to map `registry` and
   `namedRegistries` URLs to mount identities.
7. Add hosted organization mount storage namespaces and make publish/unpublish
   write to concrete hosted organization mounts.
8. Keep the root registry facade as a compatibility/default mount, but avoid
...
- tarball requests not re-running fallback after a packument selected an origin;
