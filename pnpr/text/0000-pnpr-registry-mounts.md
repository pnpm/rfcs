...
### Synthetic mount registries and lockfiles
...
The resolver should therefore return mount-routed packages as registry
resolutions plus a symbolic mount registry identity. Conceptually:
...
package-to-registry map, or another compact representation. The requirement is
that the package entry stays a **registry resolution** rather than a
`TarballResolution` with a hardcoded URL.

At install time, the client resolves `pnpr+mount:npmjs` through its current pnpr
...
IDs, no package entry changes. If no configured pnpr server can resolve the
synthetic mount registry, the install should fail with a clear configuration
error instead of falling back to the root registry or recomputing provenance.

Synthetic mount registries differ from user-authored named registries such as
`gh:`. They are generated by pnpr from mount IDs, reserved so user config cannot
shadow them, and scoped to pnpr-routed registry resolutions. User-authored
named registries still work as today for manifest specifiers and ordinary
registry selection.
...
7. Add a pnpm/pacquet lockfile representation for mount-routed registry
   resolutions so resolver output records a symbolic mount registry instead of
   a concrete tarball URL.
8. Teach install, frozen-lockfile verification, and tarball fetching to resolve
   synthetic mount registries through the current pnpr server configuration.
9. Add hosted organization mount storage namespaces and make publish/unpublish
   write to concrete hosted organization mounts.
10. Keep the root registry facade as a compatibility/default mount, but avoid
...
- pnpm/pacquet resolver output using symbolic mount registries instead of
  concrete pnpr tarball URLs;
...
- What exact lockfile shape should represent a synthetic mount registry:
  `resolution.registry`, a package-to-registry table, or another compact form?
- What should the reserved synthetic alias namespace be, and how should pnpm
  prevent user-authored named registries from shadowing it?
