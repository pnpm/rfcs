...
`GET /-/pnpr/v0/side-effects/<pkgKey>/<sideEffectsKey>` returns the manifest for one cached build: the `added` file list as `(path, digest, mode)` triples and the `deleted` path list, plus the provenance block described below. File contents are fetched as ordinary CAS blobs and verified against their digests exactly as store content is today. A digest mismatch is a cache miss, never a hard failure.
...
**Adopt the Bazel Remote Execution API v2 instead of defining an endpoint.** REAPI is a real standard with off-the-shelf servers (bazel-remote, Buildbarn, BuildBuddy), and moon adopted it after sunsetting its own hosted cache. Its data model is close to pnpm's: a content-addressable blob store plus an action cache mapping a digest to a result manifest. It was not chosen because it is gRPC-first and considerably heavier than the HTTP surface pnpm and pnpr already speak; because pnpm's payload is a *diff* rather than a full tree, which REAPI's `Directory`/`Tree` model does not express naturally; and because it carries no notion of the platform tags and provenance this proposal depends on. Attaching artifacts to registries also gets authentication and routing for free, which a REAPI deployment would need configured separately. This remains the strongest alternative and is worth revisiting if the endpoint below grows toward REAPI's shape anyway.
...
**`pnpr`.** The endpoint on the existing resolver surface alongside `/-/pnpr/v0/resolve`, capability-advertised through the `/-/pnpr` handshake. Storage in the disposable `cache` root. Reuse of `RoutePolicy` for public/private classification and of `packages:` rules for per-package policy. A publish path with its own policy verb.
...
- **Endpoint shape and naming.** The path above is a sketch. Whether the manifest and blobs are separate requests, or one response carries small files inline, is unmeasured.
