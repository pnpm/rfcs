...
**A declared builder profile.** An artifact records the environment it was produced in as a single comparable identifier rather than an open-ended property bag, and a client accepts an artifact only if its profile is one it is configured to trust.

A pinned container image digest is the concrete proposal: it fixes libc, compiler, Python, make, headers, and SDK in one identifier that matches how CI already works, and — because the build always happens at the same path inside the image — it incidentally removes a class of embedded-absolute-path problems. A profile is therefore an image digest plus a target architecture baseline plus the allowlist of environment variables passed into the build, since `CC`, `CXX`, `CFLAGS`, and `npm_config_*` still reach a build that an image alone does not constrain.

Note what this deliberately does *not* claim. It does not make builds reproducible, and it is a producer-side discipline rather than a verifiable property: nothing in an artifact proves it was built where it says. The profile makes the environment explicit and comparable; the signature in the trust section is what makes the claim attributable.

**A package eligibility contract.** Sharing is opt-in per package, not blanket. The default for a package nobody has vouched for is to build locally. Eligibility may be asserted by the publisher, by the registry operator for packages they have evaluated, or by the consuming organization for its own dependencies. A package whose build reads the clock, the network, or the host's mutable state is not eligible, and no key or profile design fixes that.

**Sharing requires behavioural equivalence, not bit-identity.** The requirement is that any artifact produced under profile P for input key K works correctly on any machine matching its compatibility constraints. Embedded timestamps, link order, and hash-iteration nondeterminism are irrelevant to that. This is a materially lower bar than Nix's, and it is why the problem is tractable at all.

A *hermetic* sandbox with declared inputs is what closes the input set rather than merely describing it — one that restricts filesystem reads, environment, network, clocks, and randomness. Plain sandboxing is not sufficient: [pnpm/pnpm#13772](https://github.com/pnpm/pnpm/issues/13772) is primarily about restricting authority and leaves network policy open, and a sandbox that permits network access does not close anything. Network is the largest residual gap after a container profile, since `node-gyp` fetches headers and `prebuild-install` fetches binaries, so hermeticity means `--network=none` plus pre-seeded caches — exactly what Nix does, and exactly what breaks packages unprepared for it. CPU feature detection is the second: the same image on hosts with and without AVX-512 can produce different code, which is why the architecture baseline belongs in the profile.

This RFC does not depend on hermetic sandboxing shipping first. The two compose, and a hermetically sandboxed build is the strongest form of an eligible one.
...
- **Provenance is signed, not merely recorded.** A signature binds the protocol version, the source tarball integrity, the input key, the builder profile, the compatibility constraints, the manifest digest, and the builder identity. Unauthenticated provenance fields are useful for debugging and useless against substitution; only the binding makes an artifact attributable.
- **The signing key must be independent of the registry.** This is the load-bearing constraint, not a deployment detail. The threat this section exists to address is a registry — or an uploader it authorized — substituting built files. A registry that signs artifacts itself defends against nothing it is not already trusted for, and reduces the signature to a transport checksum. For V1 this RFC specifies **one organization-configured CI signing key per registry, held outside the registry server**, with key identifiers, rotation, and revocation defined from the start rather than retrofitted.
...
- **Profile granularity.** An image digest plus architecture baseline plus environment allowlist is proposed, but a digest is arguably too fine: a base-image security patch changes it and invalidates every artifact, even though almost nothing about the build changed. Some notion of profile equivalence or lineage may be needed to keep hit rates usable.
