...
**Trust policy is applied at every reuse, not only at download.** An artifact whose key has since been revoked, whose registry is no longer configured for remote artifacts, or whose trust domain does not match the current project's, is not reused — it is discarded and the package builds locally. Whether this is implemented as fields on the existing entries or as a separate local index for remote artifacts is an implementation choice; the requirement is that origin survives storage and is consulted on every use.
