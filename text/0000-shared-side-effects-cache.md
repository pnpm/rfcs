...
- **Pin on first use.** When a client accepts an artifact, it records the **digest of the complete signed envelope** in the lockfile, keyed by input key and owner, exactly as it records tarball integrity.
  ...
  A lockfile is shared across platforms, so the pin is a *set*: several variants may be pinned for one input key, one per platform any contributor or CI runner has used, and a variant is accepted only if its envelope digest is among them.

  **Pins accrue on lockfile-writing installs.** A frozen install does not modify the lockfile and therefore records nothing; an artifact for a platform with no pin is accepted on signature alone, exactly as in the unpinned case below. Pins for the platforms a team actually develops on are committed by ordinary installs and protect CI thereafter; a platform only ever seen in CI stays unpinned until someone runs a non-frozen install there. Refusing unpinned artifacts on frozen installs was considered and rejected: it would disable the feature precisely where it is most valuable.
