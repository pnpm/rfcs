...
This RFC assigns (4) through (8) to pnpm, because each is a function of information pnpm holds first-hand. It leaves (1) through (3) with the host permanently: pnpm reacting to webhooks or managing VM fleets would be a product, not a package manager feature, and the hosts do those parts well. (9) is deferred, with its forward-compatibility constraints recorded below.
...
No triggers, no runners, no secret store, no web UI, no YAML dialect for expressing arbitrary jobs. A step that is not a workspace task — building a Docker image, deploying, posting to Slack — belongs to the host, or in a script a task runs. The moment `pnpm ci` grows an escape hatch for "arbitrary command with arbitrary dependencies on non-task things", it has begun re-implementing the host's job description, and the host is better at it. This boundary is what keeps the proposal a composition of two shipped RFCs rather than a product.

Distributed execution — one logical run fanned out across machines — is the genuinely tempting omission, and it is deferred rather than declined. The economics only exist because of the cache (an agent's completed task is an artifact any other agent can restore), and the coordination problem is real work: leases, work stealing, partial failure, a scheduler that spans machines. It deserves its own RFC once single-machine `pnpm ci` has proven the contract.

Three constraints are recorded now so that nothing here forecloses it, in the manner of RFC 0007's forward-compatibility section:
...
**Own more: triggers, runners, a hosted product.** The Nx Cloud / Vercel trajectory, and where the commercial gravity in this space points. Declined on scope, not on feasibility: (1)–(3) of the decomposition are a competitive product category pnpm has no business subsidizing, and every part of this proposal works identically with the hosts users already pay for. If the ecosystem ever wants a reference host, it is a separate conversation that this RFC neither starts nor forecloses.
...
- **The name.** `pnpm ci` reads perfectly and collides doubly: with `npm ci`'s muscle memory (a migrating user typing `pnpm ci` expects a frozen install, and gets one — plus a pipeline they didn't ask for, if a `default` exists), and with pnpm's own script fallthrough, under which `pnpm ci` today runs a root script named `ci`, which real workspaces have. Shadowing user scripts has precedent (`test`, `start`), but this one shadows the exact script most likely to exist. Alternatives: `pnpm ci run <pipeline>` as a namespaced subcommand, or a different verb entirely (`pnpm pipeline`?). The section headers above assume `pnpm ci`; the collision question is genuinely open.
