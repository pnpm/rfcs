...
- **Per-project overrides.** Whether they live in `package.json` or as globs in the workspace file is open. The `tasks` key name is not: it shipped as `tasks`, which is what Turborepo renamed `pipeline` to and leaves `scripts` meaning what `package.json` means by it.
