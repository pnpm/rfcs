...
- **A `--continue` flag**, whose meaning is settled and whose implementation is not. It runs every task the most recent compatible run-state journal does not record as passed, which is `--resume-from` with the anchor removed — and the anchor is exactly what the journal made unnecessary, since it exists to say what finished rather than to have it inferred from a project name. With no compatible journal it is an error rather than a silent full run: a user asking to continue is asserting there is something to continue from, and quietly running everything is the answer least likely to be what they meant. What is open is whether it earns its place next to `--resume-from`, which is a question about the CLI's surface rather than about behaviour.
