...
tasks:
  build:
    ...
    outputs: ['dist/**']
    env: ['NODE_ENV']
    inputs: ['$DEFAULT$', 'generated/schema.d.ts']
  lint:
    outputs: []
  test:
    dependsOn: ['build']
...
**An omitted `outputs` caches the logs and the exit code, and restores no files.** It is not a declaration that the task produces nothing, and it does not make the task uncacheable either — it means pnpm was not told what to keep, so it keeps what it saw. `outputs: []` means the same thing said deliberately, which is what makes `lint` worth caching.

Every comparable tool agrees on this and the agreement decided it. Turborepo: *"omitting this key or passing an empty array tells `turbo` to cache nothing (except logs, which are always cached when caching is enabled)"*. moon caches the task regardless and treats `outputs` as what enables hydration, saying of the field *"if you'd prefer to avoid that functionality, omit it"*. Nx caches too, and reaches the same place from the other side by supplying default output locations — `{projectRoot}/dist`, `build`, `public` — so an undeclared target usually still restores its files.

An earlier draft of this RFC made omission mean *not cacheable*, on the grounds that a forgotten key should not silently produce a hit that restores nothing. The failure it worried about is real, and is worst in CI where a fresh checkout has no previous output to fall back on. Three tools independently deciding otherwise is stronger evidence than one document's reasoning, and a rule that no other task runner has is a rule every migrating user trips over once. So the behaviour follows the consensus, and the failure is addressed where it actually bites: **a hit on a task with no declared outputs says so in the run output**, rather than printing a cached success indistinguishable from one that restored files. Nobody else reports this, and it costs one word on a line pnpm already prints.

Nx's mitigation — defaulting `outputs` to conventional directories rather than to nothing — remains available and is a strictly better answer for a JavaScript workspace, where `dist` really is the convention. It is not adopted here only because guessing an output location is a policy this document would rather propose on its own than smuggle in.
...
**`cache: false` opts out without withdrawing the declaration.** A task whose `outputs` exist for the orchestration RFC's benefit, or whose output is real but not worth storing — a benchmark writing timings that differ every run — sets `cache: false` and runs every time. This is deliberately not the same as omitting `outputs`: the declaration stays, and so does everything else that reads it. Its invocation-level counterpart is **`--force`**, borrowed from Turborepo along with its exact meaning: ignore existing cached artifacts and re-execute. It skips the *lookup* and not the store, which makes it the repair path as well as the bypass — a run that distrusts an entry re-runs the task and overwrites it, because a successful task is always stored — so no second flag for repair is needed.

`--no-cache` was the obvious name and is the wrong one. Turborepo has that flag and it means the opposite: *"do not cache results of the task"*, skipping the write while still reading. Defining it here as skip-the-read would hand a migrating user a familiar spelling with inverted behaviour, which is worse than an unfamiliar spelling. `--force` is unclaimed on `pnpm run` — it is an install flag, and `run` does not accept it — so the name is free and already means this to anyone arriving from Turborepo. Turborepo has since deprecated `--no-cache` in favour of a `--cache local:r,remote:rw` form, which is the shape to grow into if per-tier control is ever wanted here.

**Inputs default to the project's source files.** Every file under the project directory that git either tracks or would let you add — `git ls-files --cached --others --exclude-standard` — minus declared outputs and `node_modules`, and always the project manifest. Untracked-but-unignored files belong in the set: a source file created and not yet `git add`ed is one somebody is actively editing, and a default that skipped it would serve a stale hit for exactly the change most likely to be in flight. This matches where Nx and moon land from a different direction: both enumerate the project's files rather than asking git what is committed — moon's default is *"all files within a project (`**/*`)"* with gitignored files excluded, and Nx's is a filesystem glob over the project — so a file that exists and is not ignored counts. A workspace not in a git repository falls back to a filesystem walk honoring `.gitignore`, which is slower but not different in meaning. Explicit `inputs` entries narrow that set to the listed globs. Two spellings extend rather than replace it: the sentinel `$DEFAULT$` stands for the whole default set, and a `!`-prefixed glob excludes. Turborepo, Nx, and moon all spell exclusion with `!`, so that half is settled by everyone agreeing; the sentinel is Turborepo's `$TURBO_DEFAULT$` and Nx's `"..."` spread wearing a name that belongs to neither. So `inputs: ['$DEFAULT$', '!README.md']` is everything but the readme, and `inputs: ['$DEFAULT$', 'generated/schema.d.ts']` is the default plus one ignored file. Turborepo spells the sentinel `$TURBO_DEFAULT$`; the mechanism is worth copying and the vendor name is not.
...
**Only successful tasks are cached.** A failing task's output directory is in an unknown state and its logs are the thing the user most needs to see produced fresh. Caching failures could be added later as an opt-in without changing anything here.

This also disposes of watch-mode and other long-running tasks as far as *caching* is concerned, without a rule of their own. A `dev` task never completes, so it is never stored and never restored — the cache simply does not reach it. Nothing needs to detect that a task is long-running, and a `dev` script that someone has given `outputs` for another reason opts out with `cache: false`.

Scheduling them is a different question and not this document's. Turborepo has `persistent: true`, moon has `persistent` with a `server` preset that infers it for `dev`, `start`, and `serve`, and Nx has `continuous: true`; all three exist because a task that depends on one would otherwise wait forever. pnpm has no such declaration, so `dependsOn: ['dev']` hangs where every comparable tool raises an error. That belongs in the orchestration RFC rather than here, and three tools agreeing is reason enough to put it there.
...
**Turborepo** is the direct model and the direct competitor. `inputs`, `outputs`, `env`, `dependsOn`, `cache: false`, the `^` sigil, log replay on a hit, cache-only-on-success, `--force`'s meaning, and the `$DEFAULT$`/`!` shape for extending an input set are all its design, and this RFC adopts them because they are right and because a user migrating should be able to keep their mental model. Where the two differ, the difference is the argument:

- **An undeclared `dependsOn` keys on the dependency's packlist.** Turborepo has no default edge at all, so an undeclared task's hash contains nothing about the workspace dependency whose output it consumes.
- **The dependency component is the importer's resolved graph**, not a hash of the whole lockfile, so one team's daily dependency bump does not invalidate every project. This is the one pnpm can straightforwardly beat, and the reason is structural: pnpm already computes the graph that Turborepo can only approximate.
- **`--no-cache` is deliberately not defined**, because it exists in Turborepo with the opposite meaning.
...
Nx also corroborates the decision this document had the least company on. Its default inputs include the *dependencies'* files, so an Nx target's hash accounts for what its workspace dependencies contain even when nothing was declared — the same instinct as keying an undeclared task on its dependencies' packlists, arrived at independently. Turborepo, by contrast, has no default edge and no dependency-file component, which is exactly the hole this RFC set out to close. Nx is also the one tool that defaults `outputs` rather than leaving omission to mean "logs only", and its `continuous: true` is a third spelling of the long-running-task declaration.

**moon** is the closest thing to a third opinion on every question here, and agrees with Turborepo more often than not: tasks cache by default, `outputs` gates hydration rather than caching, `!` negates a glob, and `persistent` marks a task that never exits, with a `server` preset inferring it for `dev`, `start`, and `serve`. Its default input set is the whole project directory minus gitignored files, which is the filesystem-first reading this RFC also takes.
