...
**This is a deliberate divergence from Turborepo**, which is otherwise this design's model. There, *"omitting this key or passing an empty array tells `turbo` to cache nothing (except logs, which are always cached when caching is enabled)"* — omission and `[]` mean the same thing, and a task whose `outputs` someone forgot still takes cache hits that restore no files. That is the footgun this rule exists to close, and it is the one place where matching Turborepo would mean importing a known failure mode rather than a convention users already know.
...
- **An omitted `outputs` is not cacheable here**, where Turborepo caches logs and restores no files. Its rule turns a forgotten key into a silent hit.
