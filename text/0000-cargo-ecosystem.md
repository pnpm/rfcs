...
Python prepares a complete environment generation before publication. The coordinator settles all participants before switching `.venv`; a failed sibling leaves the existing Python environment in place. Mixed adds restore participating manifests and lockfiles through the workspace metadata transaction. Publication failures restore previous Python links, while immutable CAS entries remain reusable. Ordinary npm/Cargo installs keep their existing failure semantics. This is not a global multi-directory transaction or crash journal; older generations remain available to running processes and need a future garbage-collection policy.
...
This boundary remains demand-driven. The working Python path demonstrates that the shared network/auth/CAS contracts are usable across ecosystems, but exposes a missing lifecycle concept: returning only `Result<()>` does not express staged publication and rollback. The CLI currently retains Python's prepared generations until all installers settle. A stable adapter boundary must model that publication contract and its authoritative mutation footprint. Interpreter-dependent projections also need target identity beyond archive integrity. Resolver algorithms, manifest grammars, feature or marker evaluation, lockfile formats and projection layouts remain explicit non-goals for unification. Test and benchmark evidence lives with the implementation in `pnpm/plans/PYTHON_SPIKE.md`.
