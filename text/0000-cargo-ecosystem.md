...
Compression is not an ecosystem boundary. The implementation uses one archive-ingestion lifecycle for npm tarballs, Cargo crates, Python wheels and runtime ZIPs. Cache lookup and validation, offline gating, projection and store-index publication have one implementation. The complete download/verify/extract attempt uses one retry loop and progress tracker. Only the container decoding strategies differ: tar keeps bounded streaming extraction and ZIP keeps central-directory handling. Cross-format contract tests exercise the same cache, failure and publication rules. Cargo and Python metadata requests also use the shared network retry policy, including credential re-evaluation on every redirect and retry. Native metadata parsing and npm's conditional metadata cache remain separate contracts.
...
The workspace inventory slice in [pnpm/pnpm#14581](https://github.com/pnpm/pnpm/pull/14581) collects requested manifest basenames in one traversal. Cargo owns the interpretation of its discovered manifests and reduction to workspace roots. The inventory is not constructed on npm-only installs. Python distinguishes project manifests from tool-only `pyproject.toml` files and excludes managed environments from discovery. Configured store, cache, state, modules and virtual-store paths are also excluded before descent: cached native manifests are outputs, not workspace inputs, including on repeated installs with an in-workspace store. Each Python project owns its environment and lockfile; sharing discovery does not imply sharing a resolver root.

The Python integration in [pnpm/pnpm#14586](https://github.com/pnpm/pnpm/pull/14586) exercises the real `install`, `add`, `run` and `exec` paths. It is opt-in through `python.enabled` and routes `pypi:` selectors before npm parsing. Python owns PEP 440/508 parsing, PubGrub resolution, extras, markers, dependency groups, interpreter compatibility, RECORD validation and environment projection. The Simple JSON API uses the shared authenticated client; redirects retain Accept while reselecting credentials, and relative URLs use the final response URL. Wheel downloads use the shared integrity checks, raw CAS projection, store index and reporter. Native and pure wheels install into a usable `.venv` with entry points and `.data` relocation.

The shared `pnpm-install-coordinator` crate owns workspace locking, metadata snapshots, concurrent preparation, settlement and publication. Each native adapter supplies an `InstallTask` with its metadata footprint and returns `PreparedInstall` projections. Cargo prepares native lockfile contents and verified store slots; Python prepares complete environment generations. Both publish only after every participant settles successfully. Failed publication rolls back attempted projections in reverse order, then restores metadata. Rollback continues after an error and retains resources needed for recovery. This lifecycle is used by both ordinary install and mixed add, without ecosystem-specific branches in the coordinator.

npm explicitly enrolls as an in-place installer with its existing materialization and lifecycle-script semantics. Mixed add includes npm metadata in rollback; ordinary install does not. This does not make `node_modules` transactional or schedule npm lifecycle scripts after Cargo/Python publication. A cross-ecosystem build graph will need a distinct post-publication phase. Versioned Cargo source links and immutable CAS entries may survive failure as cache entries. The contract is in-process, not a global multi-directory transaction or crash journal; older Python generations remain available to running processes and need a future garbage-collection policy.
...
The generated single-target pylock was independently installed by uv with the same seven real PyPI distributions, including `requests[socks]` and native `orjson`. Frozen/offline replay validates resolver inputs and dependency closure; production-only installation selects a projection from the complete lockfile. The implementation is registry-wheel based: source builds, editable/local/git/URL dependencies, universal multi-target locking and arbitrary third-party pylock imports remain outside its scope, as do pip configuration and keyring discovery.

This boundary remains demand-driven and internal. The working Python path demonstrates that the shared network/auth/CAS contracts are usable across ecosystems, and the prepared-install contract now gives staged publication and authoritative metadata ownership one implementation. The lifecycle crate has no dependency on CLI configuration, an ecosystem enum, a resolver or a lockfile format. Interpreter-dependent projections still need native target identity beyond archive integrity. Resolver algorithms, manifest grammars, feature or marker evaluation, lockfile formats and projection layouts remain explicit non-goals for unification. Ownership and failure contracts are documented in `pnpm/plans/ECOSYSTEM_INSTALL.md`; the Python integration's initial test and benchmark evidence remains in `pnpm/plans/PYTHON_SPIKE.md`.
