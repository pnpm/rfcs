...
- **Atomic multi-package publish.** pnpm's batch publish (`--batch`, one `PUT /-/pnpm/v1/publish` request per registry) already solves this for registries that implement the endpoint. For npmjs-style one-request-per-package registries, should the publish step adopt the two-phase strategy discussed in [pnpm/pnpm#1098](https://github.com/pnpm/pnpm/issues/1098) (Lerna's approach): publish every package under a temporary dist-tag and flip the real dist-tag only once all packages have landed, so a partially failed release is never visible under `latest`?
- **Snapshot version base on lane-capable registries.** `0.0.0-<tag>-<timestamp>` sorts below every real release — a defensive shape that registry-lane isolation makes unnecessary. Should `--snapshot` base the version on the assembled plan's target (`2.1.0-<branch>.<timestamp>`) when publishing to a registry lane, so lane consumers see what the change will actually become?
- **Lane ↔ dist-tag ↔ registry-lane defaulting.** Should `pnpm publish` default the dist-tag (and the registry lane, where the registry supports lanes) to the package's versioning-lane name, giving the track one name from intent to install — and what does a graduated line publish under afterward (`latest`, or a maintenance tag like `v11` once a newer major owns `latest`)?
