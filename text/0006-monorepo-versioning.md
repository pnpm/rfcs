...
- **CLI commands**: `pnpm change`, `pnpm change status`, `pnpm change check`, `pnpm version -r`, `pnpm lane` (or final names), wired through the existing command infrastructure in `releasing/commands`, with config plumbed through `@pnpm/config`. The snapshot form is a `--snapshot` flag on the existing `pnpm publish -r`, which calls the same assembler and packs against the derived versions in memory.
...
- **Atomic multi-package publish.** pnpm's batch publish (`--batch`, one request per registry) already lands a whole set together on registries that implement the endpoint. For npmjs-style one-request-per-package registries, should the publish step adopt the two-phase strategy discussed in [pnpm/pnpm#1098](https://github.com/pnpm/pnpm/issues/1098) (Lerna's approach): publish every package under a temporary dist-tag and flip the real dist-tag only once all packages have landed, so a partially failed release is never visible under `latest`?
